...
export type TapestryStatus = "Not Started" | "Designed" | "In Production" | "Finished"
...
export function isValidStatus(status: string): status is TapestryStatus {
  return ["Not Started", "Designed", "In Production", "Finished"].includes(status)
}
...
function findImageInDirectory(dirPath: string, prefix = ""): string | null {
  ...
  const mainImage = files.find((file) => {
    ...
    return [".jpg", ".jpeg", ".png", ".webp"].includes(ext) && file.toLowerCase().includes("main")
  })
  ...
  const imageFile = files.find((file) => {
    const ext = path.extname(file).toLowerCase()
    return [".jpg", ".jpeg", ".png", ".webp"].includes(ext) && !file.toLowerCase().includes("thumbnail")
  })
...
function findAudioInDirectory(dirPath: string, prefix = ""): string | null {
  ...
  const audioFile = files.find((file) => {
    ...
    return (
      ...
      (file.toLowerCase().includes("audio") || file.toLowerCase().includes("description"))
    )
  })
...
export function getAllTapestries(): TapestryEntry[] {
  ...
  const tapestries = tapestryFolders
    .map((folderName) => {
      ...
      const status = data.status && isValidStatus(data.status) ? data.status : "Not Started"
      ...
      if (!thumbnail) {
        ...
        const thumbnailFile = fs.readdirSync(folderPath).find((file) => file.toLowerCase().includes("thumbnail"))
      ...
      return {
        ...
        imagePath,
        audioPath,
        audioDescription: data.audioDescription || `Audio description of the ${data.title} tapestry`,
...
export function debugTapestryColonies() {
  ...
  tapestries.forEach((tapestry) => {
    console.log(`Slug: ${tapestry.slug}, Title: ${tapestry.title}, Status: ${tapestry.status}`)
  })
  return tapestries
}
