...
export function ColonialDataExplorer({ tapestries }: ColonialDataExplorerProps) {
  // State management
  const [activeView, setActiveView] = useState<"timeline" | "grid" | "summary">("summary")
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (colony) => colony.name.toLowerCase().includes(query) || colony.summary.toLowerCase().includes(query),
      )
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "eventCount":
          ...
          break
        case "earliestEvent":
          comparison = new Date(a.earliestEvent).getTime() - new Date(b.earliestEvent).getTime()
          break
      }
  ...
  const statistics = useMemo(() => {
    const totalEvents = colonyData.reduce((sum, colony) => sum + colony.eventCount, 0)
    ...
    const earliestOverall = colonyData.reduce((earliest, colony) => {
      ...
      return earliest === "N/A" || new Date(colony.earliestEvent) < new Date(earliest) ? colony.earliestEvent : earliest
    }, "N/A")
      ...
      <Tabs value={activeView} onValueChange={(value: any) => setActiveView(value)}>
        ...
        <TabsContent value="summary" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="p-4">
              <h3 className="text-sm font-medium text-colonial-navy/70">Total Colonies</h3>
              <p className="text-2xl font-bold text-colonial-navy mt-1">{statistics.totalColonies}</p>
            </Card>
            <Card className="p-4">
              <h3 className="text-sm font-medium text-colonial-navy/70">Total Events</h3>
              <p className="text-2xl font-bold text-colonial-navy mt-1">{statistics.totalEvents}</p>
            </Card>
            <Card className="p-4">
              <h3 className="text-sm font-medium text-colonial-navy/70">Avg Events per Colony</h3>
              <p className="text-2xl font-bold text-colonial-navy mt-1">{statistics.avgEventsPerColony.toFixed(1)}</p>
            </Card>
            <Card className="p-4">
              <h3 className="text-sm font-medium text-colonial-navy/70">Earliest Event</h3>
              <p className="text-2xl font-bold text-colonial-navy mt-1">{formatDate(statistics.earliestOverall)}</p>
            </Card>
          </div>
            ...
            {filteredData.map((colony) => (
              <Card key={colony.slug} className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-lg text-colonial-navy">{colony.name}</h3>
                  <Badge variant={colony.status === "Finished" ? "default" : "outline"}>{colony.status}</Badge>
                </div>
                <p className="text-sm text-colonial-navy/70 mb-3">{colony.summary}</p>
                <div className="flex justify-between text-sm">
                  <span className="text-colonial-navy/60">Events: {colony.eventCount}</span>
                  <span className="text-colonial-navy/60">First: {formatDate(colony.earliestEvent)}</span>
                </div>
              </Card>
            ))}
          </div>
              ...
              return (
                <Card key={colony.slug} className="p-4">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-lg text-colonial-navy">{colony.name}</h3>
                      <p className="text-sm text-colonial-navy/70">{colony.summary}</p>
                    </div>
                    <Badge variant={colony.status === "Finished" ? "default" : "outline"}>{colony.status}</Badge>
                  </div>
                      ...
                      {tapestry.timelineEvents.map((event, index) => (
                        ...
                        >
                          ...
                          <div className="mt-4 text-center">
                            ...
                            <p className="text-xs text-colonial-navy/70 mt-1 max-w-[100px] truncate">{event.title}</p>
                          </div>
                        </div>
                      ))}
                    </div>
