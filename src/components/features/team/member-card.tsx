...
export function MemberCard({
  ...
  const teamImagePath = shouldUseImage ? getImageSrc(0) : placeholderPath;
    ...
    return (
      <div className={cn(`mb-4 md:mb-8 ${widthClass}`, className)}>
        <ContentCard className="overflow-hidden p-0">
          <div className="md:flex">
            <div className="md:w-1/3 lg:w-1/4 pt-6 md:pt-8">
              {/* Image Gallery for Full Variant */}
              {getImageCount() > 1 ? (
        ...
        <ImageLightbox
          ...
          onPrevious={getImageCount() > 1 ? () => setCurrentImageIndex((prev) => (prev > 0 ? prev - 1 : getImageCount() - 1)) : undefined}
          onNext={getImageCount() > 1 ? () => setCurrentImageIndex((prev) => (prev < getImageCount() - 1 ? prev + 1 : 0)) : undefined}
        />
      </div>
    );
  }
