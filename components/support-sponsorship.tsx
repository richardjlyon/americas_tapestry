...
const sponsorshipTiers: SponsorshipTier[] = [
  ...
  },
  {
    ...
    name: "Platinum",
    amount: "$25,000+",
    description: "Become a leading supporter with a Platinum level sponsorship.",
    benefits: [
  ...
  return (
    <div className="space-y-12">
        ...
        {sponsorshipTiers.map((tier) => (
          <Card
            ...
            className={`bg-white shadow-md border overflow-hidden h-full flex flex-col ${
              tier.featured ? "border-colonial-gold shadow-lg" : "border-colonial-navy/10"
            }`}
          >
            {tier.featured && (
            ...
            <CardHeader>
              <CardTitle className="flex justify-between items-start">
                <span className="text-xl text-colonial-navy">{tier.name} Sponsorship</span>
                <span className="text-lg text-colonial-burgundy font-medium">{tier.amount}</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="flex-grow">
              <p className="font-serif text-colonial-navy/80 mb-4">{tier.description}</p>
              <h4 className="font-medium text-colonial-navy mb-2">Benefits include:</h4>
              <ul className="space-y-2">
                {tier.benefits.map((benefit, index) => (
                  <li key={index} className="flex items-start">
                    <Check className="h-5 w-5 text-colonial-burgundy mr-2 flex-shrink-0" />
                    <span className="text-sm text-colonial-navy/80">{benefit}</span>
                  </li>
                ))}
              </ul>
            </CardContent>
      ...
      <div className="bg-colonial-navy/5 p-6 rounded-lg border border-colonial-navy/10 max-w-3xl mx-auto">
        <h3 className="text-xl font-bold text-colonial-navy mb-3">Custom Sponsorship Packages</h3>
        <p className="font-serif text-colonial-navy/80 mb-4">
          We understand that every organization is unique. We're happy to work with you to create a custom sponsorship
          package that aligns with your specific goals and interests. Custom sponsorships can include targeted support
          for particular tapestry panels, educational programs, or community initiatives.
        </p>
        <div className="flex flex-col sm:flex-row gap-4">
          <Button asChild className="rounded-full bg-colonial-navy text-colonial-parchment hover:bg-colonial-navy/90">
          ...
          >
            <a href="/documents/sponsorship-prospectus.pdf" target="_blank" rel="noopener noreferrer">
