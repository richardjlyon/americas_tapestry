...
const originalColonies = [
  ...
  },
  {
    ...
    name: "Pennsylvania",
    slug: "pennsylvania",
    details: "Home to Independence Hall where the Declaration of Independence was signed",
  ...
  },
  {
    ...
    name: "Rhode Island",
    slug: "rhode-island",
    details: "Founded by Roger Williams after being banished from Massachusetts",
...
export function InteractiveColoniesMap({ tapestries }: InteractiveColoniesMapProps) {
  const router = useRouter()
  const [hoveredColony, setHoveredColony] = useState<string | null>(null)
  const [selectedColony, setSelectedColony] = useState<(typeof originalColonies)[0] | null>(null)
  const [colonyTapestries, setColonyTapestries] = useState<Record<string, { slug: string; status: TapestryStatus }>>({})
  ...
  useEffect(() => {
    ...
    const loadTapestries = () => {
      try {
        ...
        const tapestryMap: Record<string, { slug: string; status: TapestryStatus }> = {}
    ...
    <div className="w-full max-w-3xl mx-auto">
      ...
      {selectedColony && (
        <div className="colony-info mt-4 p-4 border border-colonial-navy/20 rounded-md shadow-sm bg-white">
          <h3 className="text-xl font-bold text-colonial-navy">{selectedColony.name}</h3>
          <p className="font-serif text-colonial-navy/80 mt-2">{selectedColony.details}</p>
          {colonyTapestries[selectedColony.id] ? (
            <div>
              <p className="mt-2 text-sm">
                ...
                <span
                  ...
                  style={{ backgroundColor: statusColors[colonyTapestries[selectedColony.id].status] }}
                >
            ...
            <p className="mt-2 text-sm text-colonial-navy/60 italic">Tapestry not available</p>
          )}
        </div>
      )}
