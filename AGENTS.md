...
- **Code / work product:** `/Users/rjl/AIOS/Projects/americas-tapestry` (public `github.com/richardjlyon/americas_tapestry`). Stack: Next.js App Router, TypeScript, Tailwind, Jest + Playwright, deployed on Vercel; images on Cloudflare R2 (not committed to `public/`).
- **Tasks:** Plane project `TAP` (https://plane.kwlan.net, workspace `claude`). Use the `plane` skill at the AIOS root.
...
- Shop products are added via the `add-shop-product` skill (Gelato → Shopify → storefront) — don't reinvent that flow.
