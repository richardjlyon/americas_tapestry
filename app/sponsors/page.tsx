...
export default async function SponsorsPage() {
  ...
  return (
    <PageSection background="colonial-parchment">
      ...
      {Object.keys(sponsorsByTier).length > 0 && Object.entries(sponsorsByTier)
        .sort(([tierA], [tierB]) => {
          const tierOrder = { 'Platinum': 1, 'Gold': 2, 'Silver': 3, 'Bronze': 4, 'Supporter': 5 };
            ...
            {tier === 'Platinum' ? (
              <div className="max-w-2xl mx-auto">
                {tierSponsors.map(sponsor => (
                  <SponsorCard key={sponsor.slug} sponsor={sponsor} featured={true} />
                ))}
              </div>
            ) : (
      ...
      {(Object.keys(sponsorsByTier).length === 0 || untieredSponsors.length > 0) && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {(Object.keys(sponsorsByTier).length === 0 ? sponsors : untieredSponsors)
              .map(sponsor => (
                <SponsorCard key={sponsor.slug} sponsor={sponsor} />
              ))
            }
