    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {merchandiseItems.map((item) => (
          <Card key={item.id} className="bg-white shadow-md border border-colonial-navy/10 overflow-hidden">
            ...
            <CardHeader>
              <CardTitle className="flex justify-between items-start">
                ...
                <span className="text-lg text-colonial-burgundy font-medium">{item.price}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="font-serif text-colonial-navy/80">{item.description}</p>
            </CardContent>
            <CardFooter>
      ...
      <div className="text-center mt-8">
        <Button className="rounded-full bg-colonial-gold text-colonial-navy hover:bg-colonial-gold/90" size="lg">
          Visit Our Full Store <ExternalLink className="ml-2 h-4 w-4" />
        </Button>
        <p className="font-serif text-sm text-colonial-navy/60 mt-4">
          Our online store is powered by Shopify. All transactions are secure and processed through our payment
          provider.
        </p>
      </div>
    </div>
