...
export function InteractiveTimeline({ tapestries }: InteractiveTimelineProps) {
  ...
  const earliestDate = allEvents.length > 0 ? new Date(allEvents[0].date) : new Date(1600, 0, 1)
  const latestDate = allEvents.length > 0 ? new Date(allEvents[allEvents.length - 1].date) : new Date(1800, 0, 1)
  ...
  const handleScroll = () => {
    ...
    const timelineWidth = timelineRef.current.scrollWidth - timelineRef.current.clientWidth
    ...
    const filteredEvents = selectedColony ? allEvents.filter((event) => event.colonySlug === selectedColony) : allEvents
  ...
  const scrollToEvent = (event: TimelineEvent) => {
    ...
    const position =
      (getPositionFromDate(event.date) / 100) * (timelineRef.current.scrollWidth - timelineRef.current.clientWidth)
  ...
  const navigateEvent = (direction: "prev" | "next") => {
    ...
    const currentIndex = filteredEvents.findIndex(
      (event) => event.date === activeEvent.date && event.title === activeEvent.title,
    )

    if (currentIndex === -1) return

    const newIndex =
      direction === "next" ? Math.min(currentIndex + 1, filteredEvents.length - 1) : Math.max(currentIndex - 1, 0)
  ...
  const filterByColony = (colonySlug: string | null) => {
    ...
    const colonyEvents = allEvents.filter((event) => event.colonySlug === colonySlug)
  ...
  return (
    <div className="w-full flex flex-col">
        ...
        {activeEvent ? (
          <div className="flex flex-col">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="text-xl md:text-2xl font-bold text-colonial-navy">{activeEvent.title}</h3>
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  disabled={filteredEvents.indexOf(activeEvent) === filteredEvents.length - 1}
                  className="h-8 w-8 p-0 rounded-full"
            ...
            <p className="font-serif text-colonial-navy/80 mt-2">{activeEvent.description}</p>
            <div className="mt-4">
              <Link
      ...
      >
        {/* Timeline Bar */}
        <div className="absolute h-2 bg-colonial-navy/30 top-16" style={{ width: "100%", minWidth: "2000px" }}>
          {/* Timeline Events */}
            ...
            >
              <button
                ...
                className={cn(
                  ...
                  activeEvent && activeEvent.date === event.date && activeEvent.title === event.title
              ...
              <div className="absolute whitespace-nowrap -mt-8 transform -translate-x-1/2 left-1/2">
                <p className="text-xs font-medium text-colonial-navy">{new Date(event.date).getFullYear()}</p>
              </div>
              <div className="absolute whitespace-nowrap mt-8 transform -translate-x-1/2 left-1/2 max-w-[150px] overflow-hidden text-ellipsis">
                <p className="text-xs text-colonial-navy/80 text-center">{event.title}</p>
              </div>
            </div>
          ))}
        </div>
